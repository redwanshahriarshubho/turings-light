/* ═══════════════════════════════════════════════════════════
   ...
   COMPLETELY REWRITTEN — fixes Act II double loop
═══════════════════════════════════════════════════════════ */
...
const Engine = (() => {
  ...
  function levelComplete({ icon, title, body, isFinal = false }) {
    ...
    modalCallback = () => {
      ...
      const nextIdx = currentStoryIdx + 1;
      console.log("Level complete. currentStoryIdx:", currentStoryIdx, "→ nextIdx:", nextIdx, "STORY.length:", STORY.length);
  ...
  function startTitleCanvas() {
    ...
    const stars = Array.from({ length: 260 }, () => ({
      ...
      phase: Math.random()*Math.PI*2, spd: Math.random()*0.02+0.006,
    }));
    const bands = [
      {c:"rgba(255,68,68,",y:0.72,h:0.05},{c:"rgba(255,146,0,",y:0.77,h:0.04},
      {c:"rgba(255,230,0,",y:0.81,h:0.04},{c:"rgba(0,230,118,",y:0.85,h:0.04},
      {c:"rgba(41,121,255,",y:0.89,h:0.04},{c:"rgba(204,68,255,",y:0.93,h:0.04},
    ];
    let t = 0;
    function draw() {
      t += 0.012;
      ...
      stars.forEach(s => {
        ...
        ctx.fillStyle=`rgba(255,255,255,${s.a*(0.5+0.5*Math.sin(s.phase))})`; ctx.fill();
      });
      bands.forEach(b => {
        const wave = Math.sin(t+b.y*10)*0.008;
        const g = ctx.createLinearGradient(0,0,W,0);
        g.addColorStop(0,b.c+"0)"); g.addColorStop(0.3,b.c+"0.18)");
        g.addColorStop(0.5,b.c+(0.28+0.08*Math.sin(t)).toFixed(2)+")");
        g.addColorStop(0.7,b.c+"0.18)"); g.addColorStop(1,b.c+"0)");
        ctx.fillStyle=g; ctx.fillRect(0,(b.y+wave)*H,W,b.h*H);
      });
  ...
  function startStoryCanvas() {
    ...
    function draw() {
      ...
      lamp.addColorStop(0.4,"rgba(255,150,0,0.04)"); lamp.addColorStop(1,"rgba(0,0,0,0)");
  ...
  function startBgCanvas() {
    ...
    const bits=Array.from({length:30},()=>({
      ...
      char:Math.random()<0.5?"0":"1",alpha:Math.random()*0.3,size:10+Math.random()*8,
    }));
    function draw() {
      t+=0.005;
      const W=canvas.width,H=canvas.height;
      ...
      sky.addColorStop(0,`rgb(${r},${g},${b})`); sky.addColorStop(1,`rgb(${r+4},${g+6},${b+8})`);
      ctx.fillStyle=sky; ctx.fillRect(0,0,W,H);
      const sunG=ctx.createRadialGradient(W*0.85,H*0.12,0,W*0.85,H*0.12,180);
      ...
      ctx.fillStyle=sunG; ctx.beginPath(); ctx.arc(W*0.85,H*0.12,180,0,Math.PI*2); ctx.fill();
      bits.forEach(bit=>{
        bit.x+=bit.vx; bit.y+=bit.vy;
        if(bit.y<-20){bit.y=H+10;bit.x=Math.random()*W;}
        if(bit.x<-20)bit.x=W+10; if(bit.x>W+20)bit.x=-10;
  ...
  function startEndingCanvas() {
    const canvas=$("ending-canvas"); if(!canvas) return;
    const ctx=canvas.getContext("2d");
    canvas.width=window.innerWidth; canvas.height=window.innerHeight;
    ...
    function draw() {
      ...
      prideC.forEach(([r,g,b],i)=>{
        const yBase=H*(0.25+i*0.09),wave=Math.sin(t+i*0.9)*H*0.04;
  ...
  window.addEventListener("resize",()=>{
    ["title-canvas","story-canvas","bg-canvas","ending-canvas"].forEach(id=>{
      const c=$(id); if(c){c.width=window.innerWidth;c.height=window.innerHeight;}
    });
  });
